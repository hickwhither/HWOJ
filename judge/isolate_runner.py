...
def isolate_run(
    ...
    with isolate_sandbox(box_id, use_cgroups) as box_dir:
        # 1. Define Files
        box_exec = os.path.join(box_dir, exec_path)
        ...
        if os.path.exists("meta.txt"):
            with open("meta.txt", "r", encoding="utf-8", errors="replace") as f:
                for line in f:
                    if ":" in line:
                        k, v = line.strip().split(":", 1)
                        meta[k] = v
            os.remove("meta.txt") # Đọc xong xóa luôn file meta bên ngoài
